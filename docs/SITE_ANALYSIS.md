...
- `src/app`：根壳层、Provider 与唯一的路由表
- `src/features`：页面与业务能力，按 home、profile、projects、blog、friends、fandom、errors、search、github、assistant 分组
- `src/shared/components`：跨业务复用的布局、控件、UI 与视觉效果
- `src/data`：个人信息、项目、奖项、博客、专题、友链等静态数据
- `src/posts`：Markdown 博客正文
- `src/hooks`：页面元信息、主题等 Hook
- `src/contexts`：主题上下文
- `src/features/assistant`：AI 助手视图、知识库、检索与 OpenAI 兼容 API 适配
- `src/utils`：统计与性能日志
...
flowchart LR
  ...
  K[src/features/assistant/knowledgeBase.ts] --> AI[AiAssistant]
```
...
1. 在所属 `src/features/<name>` 添加页面组件。
2. 在 `src/app/routes.tsx` 添加 lazy import 和 `<Route>`。
